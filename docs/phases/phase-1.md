...
### Project Setup
- [ ] Section Complete
- [ ] T001 Create Cargo workspace (e.g. `voxel-game`) with binary target; pin Bevy version in `Cargo.toml` per `@docs/architecture.md`.
- [ ] T002 Add dependencies: `bevy`, `serde`, `bincode`, `glam`, `rand`, `noise`, `tracing` (or use Bevy’s re-exports where applicable); ensure no extra voxel/networking crates yet.
