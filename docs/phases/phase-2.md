...
- [ ] Section Complete
- [ ] T012 Add `bevy_voxel_world` (and its dependencies, e.g. `block-mesh`) to `Cargo.toml`; ensure version is compatible with pinned Bevy.
- [ ] T013 Register the bevy_voxel_world plugin in the Bevy app and configure chunk size / spawn range so the world loads around the origin.
